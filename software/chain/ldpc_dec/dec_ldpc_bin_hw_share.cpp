	...
	for(unsigned int cng_counter = 0; cng_counter < num_check_nodes_ / dst_parallelism_; cng_counter++)
	{
		...
		for(unsigned int cfu_counter = 0; cfu_counter < dst_parallelism_; cfu_counter++)
		{
            ...
            case SPLIT_ROW_SELF_CORRECTING:
                Check_Node_Split_Row(check_node_io, esf_factor_, num_partitions_, threshold_, cng_counter, edges_threshold_); break;
			}
                                                                     ...
                                                                     int             iter)
{
	...
	for(unsigned int cng_counter = 0; cng_counter < num_check_nodes_ / dst_parallelism_; cng_counter++)
	{
		...
		for(unsigned int cfu_counter = 0; cfu_counter < dst_parallelism_; cfu_counter++)
		{
            ...
            case SPLIT_ROW_IMPROVED:
            case SPLIT_ROW_SELF_CORRECTING:
                Check_Node_Split_Row(check_node_io[cfu_counter], esf_factor_, num_partitions_, threshold_, cng_counter, edges_threshold_);
                break;
			}
                                                            ...
                                                            bool            app_parity_check)
{
	...
	for(unsigned int cng_counter = 0; cng_counter < num_cng; cng_counter++) {
		...
		for(unsigned int cfu_counter = 0; cfu_counter < dst_parallelism_; cfu_counter++)
            ...
            case SPLIT_ROW_IMPROVED:
            case SPLIT_ROW_SELF_CORRECTING:
                Check_Node_Split_Row(check_node_io[cng_counter][cfu_counter], esf_factor_, num_partitions_, threshold_, cng_counter, edges_threshold_);
                break;
			}
    }
...
int Decoder_LDPC_Binary_HW_Share::Check_Node_Split_Row(Buffer<int> &in_out_msg, float esf_factor, unsigned int partitions, unsigned int threshold, unsigned int cng_counter, unsigned int edges_threshold)
{
    ...
    */
    ...
    real_edges = Calculate_Partition_Length(partition_length, cng_counter, partitions);

    if (real_edges < edges_threshold) {
//        cout << "for row: " << cng_counter << " use Check_Node_Min_Sum" << endl;
        pchk_sign = Check_Node_Min_Sum(in_out_msg, esf_factor);
    } else {
 //       cout << "for row: " << cng_counter << "use Check_Node_Split_Row" << endl;
...
unsigned int Decoder_LDPC_Binary_HW_Share::Calculate_Partition_Length(unsigned int partition_length[], unsigned int cng_counter, unsigned int partitions)
{
    unsigned int rest;
