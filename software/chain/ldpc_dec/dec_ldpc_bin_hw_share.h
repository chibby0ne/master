...
protected:
    ...
    // number of edges that serves as threshold for deciding whether or not to split row
    unsigned int edges_threshold_;
	 ...
	 * \return  1 : Parity check was satisfied.
	 */
	int Check_Node_Split_Row(Buffer<int> &in_out_msg, float esf_factor, unsigned int num_partitions, unsigned int threshold, unsigned int cng_counter, unsigned int edges_threshold);

    unsigned int Calculate_Partition_Length(unsigned int partition_length[], unsigned int cng_counter, unsigned int partitions);
